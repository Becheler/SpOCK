...
height_fig = 11.0  # the width is calculated as height_fig * 4/3.
fontsize_plot = 25
ratio_fig_size = 4.0 / 3
color = [
    "lime",
    "blue",
    "indigo",
...
output_file_path_list = var_in[
    find_in_read_input_order_variables(var_in_order, "output_file_path_list")
]
output_file_name_list = var_in[
    find_in_read_input_order_variables(var_in_order, "output_file_name_list")
]
...
for ivar in range(nvar):
    ...
    for isc in range(nb_sc):
        if ivar == 0:
            var_out, var_out_order = read_output_file(
                output_file_path_list[isc] + output_file_name_list[isc], var_to_read
            )
            if isc == 0:  # same date for all sc
                date = var_out[
                    find_in_read_input_order_variables(var_out_order, "date")
                ]
                nb_seconds_since_start = var_out[
                    find_in_read_input_order_variables(
                        var_out_order, "nb_seconds_since_start"
                    )
                ]
                nb_steps_new = len(
                    date
                )  # in case the sc reentered the atmosphere before the end of the run
                date_ref = datetime.strptime(date[0], "%Y/%m/%d %H:%M:%S.%f")
                nb_seconds_in_simu = (nb_steps_new - 1) * dt
                altitude = np.zeros(
                    [nb_sc, nb_steps_new]
                )  # all output files of one simulation have the same number of steps
                latitude = np.zeros([nb_sc, nb_steps_new])
                longitude = np.zeros([nb_sc, nb_steps_new])
                x_axis = np.arange(0, nb_seconds_in_simu + 1, dt)
            altitude[isc, :nb_steps_new] = var_out[
                find_in_read_input_order_variables(var_out_order, "altitude")
            ]
            latitude[isc, :nb_steps_new] = var_out[
                find_in_read_input_order_variables(var_out_order, "latitude")
            ]
            longitude[isc, :nb_steps_new] = var_out[
                find_in_read_input_order_variables(var_out_order, "longitude")
            ]
        ...
        if isc == 0:
            ...
            fig = plt.figure(
                num=None,
                figsize=(height_fig * ratio_fig_size, height_fig),
                dpi=80,
                facecolor="w",
                edgecolor="k",
            )
            ...
            fig.suptitle(
                fig_title,
                y=0.965,
                fontsize=(int)(fontsize_plot * 1.1),
                weight="normal",
            )
            ...
            ]  # change the width of the frame of the figure
            ax.tick_params(
                axis="both",
                which="major",
                labelsize=fontsize_plot,
                size=10,
                width=2,
                pad=7,
            )
        ...
        ax.plot(
            x_axis, y_axis, linewidth=2, color=color[isc], label="SC #" + str(isc + 1)
        )
        ...
        if isc == nb_sc - 1:
            ...
            nb_ticks_xlabel = 8
            dt_xlabel = (
                nb_seconds_in_simu / nb_ticks_xlabel
            )  # dt for ticks on x axis (in seconds)
            ...
            for i in range(len(xticks)):
                if dt_xlabel >= 3 * 24 * 3600:
                    ...
                    date_list_str.append(
                        str(date_list[i])[5:10] + "\n" + str(date_list[i])[11:16]
                    )
